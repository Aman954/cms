...
$usersec=$_SESSION['username'];
 ...
 if(isset($_GET['edit'])){
   ...
   if(mysqli_num_rows($rt)>0)
   {
     ...
     $uusername=strtolower($getv['username']);
   }
...
<div class="col-lg-9 col-md-12 col-sm-12" style="width:auto;">
  <i class="fa fa-2x fa-user" aria-hidden="true" style="color:#2fa4e7;">&nbsp;Update Profile <small style="color:#495057;opacity:0.9;">Update User Profile</small></i>
<hr>
<ol class="breadcrumb">
  <li class="breadcrumb-item "><i class="fa fa-tachometer" aria-hidden="true";>&nbsp;Dashboard</i></li>
  <li class="breadcrumb-item active"><i class="fa fa-user" aria-hidden="true">&nbsp;Update Profile</i></li>
 ...
 $password_new=crypt($add_pass,$pass_crypt);
  ...
  if(empty($add_first_name) or empty($add_last_name) )
  {
    $error_msg="All (*) fields are required";
      ...
      $update_query="UPDATE `users` SET `first_name` = '$add_first_name', `last_name` = '$add_last_name',  `password` = '$password_new', `details` = '$details'
";
       if(isset($add_image))
      {
     ...
     if(mysqli_query($conn,$update_query))
     {
       ...
       header("refresh:0;url=edituser.php?edit=$uid");
     }
...
<form action='' method='POST' style='margin-bottom:40px;font-weight:bold;' enctype='multipart/form-data'>
  ...
  <input type='submit' value='Update profile' class='btn btn-primary' name='submit-btn'>
...
echo "<img src='images/$uimage' alt='user' style='max-width:100%;border:4px groove grey;'></img>";
